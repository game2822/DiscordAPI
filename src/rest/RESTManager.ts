...
export class RestManager {
    ...
    private async sendRequest<T>(options: RequestOptions,textonly?: boolean): Promise<T> {
        ...
        let requestHeaders: Record<string, string> = {
          "User-Agent": "@game2822/DiscordAPI (tempname)",
          ...headers,
        };
        ...
        if (textonly === undefined || textonly === false) {
            ...
            if (responseCookies) {
                (this as any).cookies = responseCookies;
            }
    ...
    async delete<T>(path: string, params?: Record<string, any>, options?: RequestOptions, textonly?: boolean): Promise<T> {
